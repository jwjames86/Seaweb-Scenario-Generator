...
function normalizedPastGuestFlags(count,flags,values){
...
function syncLatitudesGuestCard(card){
  const toggle=card.querySelector('.past-guest-toggle');
  const wrap=card.querySelector('.latitudes-number-wrap');
  const note=card.querySelector('.latitudes-guest-note');
  const input=card.querySelector('.latitude-number-input');
  const isPast=!!toggle?.checked;
  wrap?.classList.toggle('hidden-field',!isPast);
  card.classList.toggle('is-past-guest',isPast);
  card.classList.toggle('is-new-guest',!isPast);
  if(note) note.textContent=isPast ? 'Past guest — use the training Latitudes number below.' : 'New guest — no Latitudes number is needed.';
...
function renderLatitudesFields(values,flags){
  ...
  const existingFlags=normalizedPastGuestFlags(count,Array.isArray(flags)?flags:collectPastGuestFlags(),existingValues);
  ...
  host.innerHTML=Array.from({length:count},(_,i)=>{
    ...
    const checked=!!existingFlags[i];
    return `<div class="latitudes-guest-card ${checked?'is-past-guest':'is-new-guest'}">
      <div class="latitudes-guest-head">
        <strong>${escapeHtml(guest)}</strong>
        <label class="check compact"><input id="pastGuest${i+1}" class="past-guest-toggle" type="checkbox" ${checked?'checked':''} /> Past guest</label>
      </div>
      <div class="latitudes-guest-note">${checked?'Past guest — use the training Latitudes number below.':'New guest — no Latitudes number is needed.'}</div>
      <label class="latitudes-number-wrap ${checked?'':'hidden-field'}"><span>Latitudes number</span><input id="latitudeNumber${i+1}" class="latitude-number-input" inputmode="numeric" autocomplete="off" maxlength="12" placeholder="Latitudes number" value="${escapeAttr(value)}" /></label>
    </div>`;
  }).join('');
  host.querySelectorAll('.past-guest-toggle').forEach(toggle=>{
    const card=toggle.closest('.latitudes-guest-card');
    const sync=()=>syncLatitudesGuestCard(card);
...
function getGuestProfileMix(d){
  ...
  const newCount=Math.max(0,count-pastCount);
  const numbersProvided=flags.reduce((n,isPast,i)=>n+(isPast && numbers[i]?1:0),0);
  return {count,flags,numbers,pastCount,newCount,numbersProvided};
}
...
function atAGlanceHtml(d,meta){
  ...
  const guestSummary=d.latitudes
    ? `${guestMix.pastCount} past guest${guestMix.pastCount===1?'':'s'}${guestMix.newCount?` • ${guestMix.newCount} new guest${guestMix.newCount===1?'':'s'}`:''}`
    : 'Create or verify profiles';
  return `<div class="glance-grid">
    <div class="glance-card"><span>Guests</span><strong>${d.guestCount} ${d.guestCount===1?"guest":"guests"}</strong><small>${escapeHtml(guestSummary)}</small></div>
...
function latitudesScenarioHtml(d){
  ...
  const rows=Array.from({length:d.guestCount},(_,i)=>{
    ...
    return `<div class="latitudes-output-card"><span>Guest ${i+1}</span><strong>${escapeHtml(name)}</strong><small>${isPast?`Past guest${number?` • Latitudes #: ${escapeHtml(number)}`:` • <em>Latitudes number not entered — verify in Seaweb</em>`}`:'New guest • Create a new training guest profile in Seaweb.'}</small></div>`;
  }).join('');
  return `<section class="scenario-section latitudes-output-section"><div class="section-label">GUEST STATUS / LATITUDES</div><h3>Guest Status & Latitudes</h3><p class="latitudes-output-intro">Use this section to determine which guests should be located as past guests in Seaweb and which guests should be created as new profiles.</p><div class="latitudes-output-grid">${rows}</div></section>`;
}
...
function fullTaskList(d,meta){
  ...
  const guestMix=getGuestProfileMix(d);
  tasks.push(d.latitudes?(guestMix.pastCount && guestMix.newCount?"Use the guest-status section to identify which guests are past guests and which are new. Locate or create each profile as directed, then confirm legal names and dates of birth.":guestMix.pastCount?"Use the provided Latitudes number(s) to locate/verify the past-guest profile(s), then confirm legal names and dates of birth.":"Create the new guest profile(s) shown in the scenario and confirm legal names and dates of birth."):"Create or verify all required guest profiles using the training details.");
...
function generateScenario(){
  ...
  const guestInfoItems=Array.from({length:d.guestCount},(_,i)=>{
    const label=guestLabel(i);
    if(d.latitudes){
      return `<li><strong>Guest ${i+1}:</strong> ${escapeHtml(label)} — ${guestMix.flags[i]?`Past guest${guestMix.numbers[i]?` • Latitudes #: ${escapeHtml(guestMix.numbers[i])}`:' • verify Latitudes number in Seaweb'}`:'New guest • create a new training guest profile in Seaweb'}</li>`;
    }
    return `<li><strong>Guest ${i+1}:</strong> ${escapeHtml(label)}</li>`;
  }).join('');
...
function runValidator(){
  ...
  if(d.latitudes){
    const mix=getGuestProfileMix(d);
    const missing=mix.flags.reduce((arr,isPast,i)=>{ if(isPast && !mix.numbers[i]) arr.push(i+1); return arr; },[]);
    if(!mix.pastCount) add("warning","No past guests selected","Past Guests / Latitudes is turned on, but every guest is currently marked as a new guest.");
    else if(missing.length) add("warning","Some past guests are missing Latitudes numbers",`Add a training Latitudes number for Guest ${missing.join(', Guest ')} or mark that guest as new.`);
    else add("passed","Guest status and Latitudes set",`${mix.pastCount} past guest${mix.pastCount===1?"":"s"}${mix.newCount?` and ${mix.newCount} new guest${mix.newCount===1?"":"s"}`:''} configured for the trainee scenario.`);
  }
...
function makeTeamsPageClones(){
  ...
  contentNodes.forEach(node=>{
    ...
    if(label==='YOUR CALL' || label==='GUEST REQUEST' || label==='GUEST STATUS / LATITUDES' || label==='YOUR WORK' || clsHas(node,'latitudes-output-section')){
      groups[0].push(node);
    }else if(label==='REFERENCE DETAILS' || label==='TRAINING PAYMENT' || heading.includes('CALL FLOW SUPPORT') || clsHas(node,'details-section') || clsHas(node,'payment-section')){
      groups[1].push(node);
    }else{
      groups[2].push(node);
    }
  ...
  const titles=[
    'Guest Scenario, Request & Tasks',
    'Reference Details & Booking Workflow',
    'Final Check & Knowledge Review'
  ];
  const subtitles=[
    'Customer scenario, at-a-glance brief, guest status and trainee tasks',
    'Call-flow support, reservation reference details and payment guidance',
    'Required closing actions and after-completion knowledge review'
  ];
...
async function downloadTeamsPageSet(){
  ...
  btn.disabled=true;
  btn.innerHTML=`<span class="share-menu-icon">…</span><span><strong>Building Teams pages…</strong><small>Creating 8.5 × 14 PNG pages</small></span>`;
  try{
...
async function downloadScenarioPdf(){
  ...
  try{
    ...
    setTimeout(()=>URL.revokeObjectURL(url),2000);
    closeShareMenu();
    flash(`PDF downloaded with ${pages.length} page${pages.length===1?'':'s'}. The PDF uses the exact same page design as the PNG export on legal-size pages.`);
  }catch(err){
